<template>
    <div>
        ...
        <product-form v-if="this.action=='show'" v-bind:product="addOrUpdateProduct"
         v-on:add-product="addProduct" v-bind:manufacturers="this.manufacturers" v-on:update-product="editProduct" v-on:action-show="actionShow"></product-form>
    </div>
</template>
...
export default {
    ...
    watch:{
        ...
        'temp_products':function(new_v,old_v){
            if(JSON.stringify(old_v) !== JSON.stringify(new_v)){
    ...
    methods: 
    {
        ...
        getProducts(){
            ...
            .then(({data}) => {
                this.temp_products = [...data]
                this.pages.total = this.totalPagesFiltered;
                // alert(this.pages.total) 
                this.sort.type = 'id';
